...
Remote reproduction notes:

Run these commands on the remote host. No `ssh` prefix is shown here.
For future runs, start the container setup and inspection commands inside named
...
Run the inspection inside the container:
...
Copy the generated log out to the remote host checkout:
